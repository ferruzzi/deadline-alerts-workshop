...
**This file is deliberately incomplete, and the checker will tell you so:**
    ...
    - CloseOfBusinessDeadline loses field(s) cob_variable_name when serialized.

That is the expected result here, not a mistake in the solution.  The class now has a field, but it still uses the
inherited ``serialize_reference`` and ``deserialize_reference``, which carry only the class name.  Anything the
scheduler rebuilds from the serialized Dag therefore comes back with that field reset to its default.

**One key takeaway here** is that Airflow will not complain either.  The demo Dag passes exactly the same Variable
name as the default, so resetting it changes nothing observable; it runs, the deadline is correct, and the logs
are clean.  Point the Reference at a different Variable and it quietly keeps using the default instead.

Step 6 adds the serializers and closes that gap.  Diff this file against ``cob_reference_step6_final.py`` to see
